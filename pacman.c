...
//For colors
enum { Wall = 1, Normal, Pellet, PowerUp, GhostWall, Ghost1, Ghost2, Ghost3, Ghost4, Ghost5, BlueGhost, Pacman };

//I know global variables are bad, but it's just sooo easy!
int Loc[6][2] = { 0 };					//Location of Ghosts and Pacman
int Dir[6][2] = { 0 };					//Direction of Ghosts and Pacman
int StartingPoints[6][2] = { 0 };			//Default location in case Pacman/Ghosts die
...
void CheckCollision() {
	...
	for(a = 0; a < 5; a++) {
		//Collision
		if((Loc[a][0] == Loc[5][0]) && (Loc[a][1] == Loc[5][1])) {

			//Ghost dies
			if(Invincible == 1) {
				Points = Points + GhostsInARow * 20;
				mvwprintw(win, Loc[5][0], Loc[5][1] - 1, "%d", (GhostsInARow * 20));
				GhostsInARow *= 2;
				wrefresh(win);
			...
			//Pacman dies
				...
				mvwprintw(win, Loc[5][0], Loc[5][1], "X");
...
void DrawWindow() {
	...
	//Display Pacman
	wattron(win, COLOR_PAIR(Pacman)); mvwaddch(win, Loc[5][0], Loc[5][1], 'C');
...
void GetInput() {
	...
	switch (ch) {
		case KEY_UP:    case 'w': case 'W':
			if((Level[(Loc[5][0]-1) % 39][Loc[5][1]] != 1)
			&& (Level[(Loc[5][0]-1) % 39][Loc[5][1]] != 5))
				{ Dir[5][0] = -1; Dir[5][1] =  0; }
			break;
		...
		case KEY_LEFT:  case 'a': case 'A':
			if((Level[Loc[5][0]][(Loc[5][1]-1) % 38] != 1)
			&& (Level[Loc[5][0]][(Loc[5][1]-1) % 38] != 5))
				{ Dir[5][0] =  0; Dir[5][1] = -1; }
			break;
	...
	for(a = 0; a < 5; a++) {
		...
		do {
			...
			if(checksides[b] == 1) {
				...
				if(Invincible == 0) {
				//Chase Pacman
				     if((Loc[5][0] > Loc[a][0]) && (checksides[0] == 1)) { Dir[a][0] =  1; Dir[a][1] =  0; c = 1; }
				else if((Loc[5][0] < Loc[a][0]) && (checksides[1] == 1)) { Dir[a][0] = -1; Dir[a][1] =  0; c = 1; }
				else if((Loc[5][1] > Loc[a][1]) && (checksides[2] == 1)) { Dir[a][0] =  0; Dir[a][1] =  1; c = 1; }
				else if((Loc[5][1] < Loc[a][1]) && (checksides[3] == 1)) { Dir[a][0] =  0; Dir[a][1] = -1; c = 1; }
				}

				else {
				//Run away from Pacman
				     if((Loc[5][0] > Loc[a][0]) && (checksides[1] == 1)) { Dir[a][0] = -1; Dir[a][1] =  0; c = 1; }
				else if((Loc[5][0] < Loc[a][0]) && (checksides[0] == 1)) { Dir[a][0] =  1; Dir[a][1] =  0; c = 1; }
				else if((Loc[5][1] > Loc[a][1]) && (checksides[3] == 1)) { Dir[a][0] =  0; Dir[a][1] = -1; c = 1; }
				else if((Loc[5][1] < Loc[a][1]) && (checksides[2] == 1)) { Dir[a][0] =  0; Dir[a][1] =  1; c = 1; }
				}
			}
...
void MovePacman() {
	...
	//Switch sides?
	     if((Loc[5][0] ==  0) && (Dir[5][0] == -1)) Loc[5][0] = 38;
	else if((Loc[5][0] == 38) && (Dir[5][0] ==  1)) Loc[5][0] =  0;
	else if((Loc[5][1] ==  0) && (Dir[5][1] == -1)) Loc[5][1] = 37;
	else if((Loc[5][1] == 37) && (Dir[5][1] ==  1)) Loc[5][1] =  0;
	...
	//Or
		...
		if((Level[Loc[5][0]][Loc[5][1]] == 1) || (Level[Loc[5][0]][Loc[5][1]] == 5)) {
			Loc[5][0] -= Dir[5][0];	Loc[5][1] -= Dir[5][1];
		}
	}

	//What is he eating?
	switch (Level[Loc[5][0]][Loc[5][1]]) {
		case 2:	//Pellet
